...
$dbname = "SJL";
  ...
  $user = $_SESSION['uid'];
   ...
   $credits_sum = $db->query("SELECT sum(c.credits) as sum_of_credits from course c where c.crn = 1 or c.crn = 3 or c.crn = 5");
...
<head>
<title>View Advising Form</title>
...
<?php
  ...
  if($credits_sum<9){
    //echo $credits_sum;
    echo "<b>ERROR: You need at least 9 credits to lift your registration hold. Please fill out the advising form with the necessary credentials.</b></br>";
  ...
  <?php
    ...
    $course_array = $db->query("SELECT subject, course_num FROM advHold where university_id =".  $_SESSION['uid']);
