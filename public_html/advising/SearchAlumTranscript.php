...
<?php
  ...
  $info_array = $db->query("SELECT DISTINCT f_name, l_name, program_type, grad_year, grad_semester, advisor.name as aname
    ...
    if (!empty($info_array)) {
      ...
      while($row2 = $info_array->fetch_assoc())
      {
    ?>
  <tr>
    <td><?php echo $row2["f_name"]; ?> <?php echo " ". $row2["l_name"]; ?></td>
    <td><?php echo $row2["program_type"]; ?></td>
    <td><?php echo $row2['aname']; ?></td>
    <td><?php echo $row2['grad_semester'];?><?php echo $row2['grad_year'];?></td>
  </tr>
...
if($row['final_grade'] == 'A'){
      ...
      $GPA = $sum/$credits_sum;
      mysqli_query($conn,"update alumni set GPA = '$GPA' where university_id =". $university_id);
